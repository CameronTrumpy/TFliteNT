######## USB Webcam Object Detection with Raspberry Pi + Tensorflow Lite#########
#
# Author: github.com/CameronTrumpy
# Date: 07/01/21
# Description: 
# Utilizing TensorFlow Lite, the Pi runs object detection with a USB Webcam. Then with OpenCV draws boxes and scores around the objects of interest in each frame from the
# webcam. To improve FPS, the webcam object runs in a separate thread from the main program.
# This script expects a regular USB webcam.
#
# Original source material from TensorFlow Lite image classification example:
...
#Init NT server with configured IP address of server
...
parser.add_argument('--edgetpu', help='Use Coral Edge TPU Accelerator to speed up detection',
                    action='store_true')
parser.add_argument('--preview', help='Display image preview with labels + bounding boxes drawn',
                    action='store_true')
    ...
    for i in range(len(scores)):
        if ((scores[i] > min_conf_threshold) and (scores[i] <= 1.0)):
            ...
            if show_Preview: #draw bounding box
                ...
                cv2.putText(frame,'FPS: {0:.2f}'.format(frame_rate_calc),(30,50),cv2.FONT_HERSHEY_SIMPLEX,1,(255,255,0),2,cv2.LINE_AA) # draw framerate in image corner
